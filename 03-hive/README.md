...
Nota: Esta tabla la crea en una BASE DE DATOS 'terpel##'
```
> use terpel##;
> show tables;
> describe hdi;
...
```
> load data local inpath '/home/terpel##/datasets/onu/hdi-data.csv' into table HDI;
```
...
```
> use <username>;
>CREATE EXTERNAL TABLE docs (line STRING) STORED AS TEXTFILE LOCATION '/user/terpel##/datasets/gutenberg/gutenberg-small/';
```
...
```
$ sqoop create-hive-table --connect jdbc:mysql://127.0.0.1:3306/cursodb --username curso -P --table employee --hive-database terpel## --hive-table employee -m 1--mysql-delimiters
```

// Transferir datos de una base de datos (tipo mysql) hacia HIVE vía HDFS:

```
$ sqoop import --connect jdbc:mysql://127.0.0.1:3306/cursodb --username curso -P --table employee --hive-import --hive-database terpel## --hive-table employee -m 1 --mysql-delimiters
```
